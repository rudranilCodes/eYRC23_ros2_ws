    ...
    if len(corners) > 0:
        ...
        for (markerCorner,markerID) in zip(corners,ids):
            ...
            if(a>aruco_area_threshold):
                ...
                width_aruco_list.append(w)
                angle_aruco_list.append(yaw_pitch_roll[0][2])
...
class aruco_tf(Node):
    ...
    def __init__(self):
        ...
        self.publisher_1 = self.create_publisher(String, 'rudranil', 50)
    ...
    def process_image(self):
        ...
        Returns:
        '''
        center_list,distance_list,angle_list,width_list,id_list=detect_aruco(self.cv_image)
        msg = String()
        ############ Function VARIABLES ############
        ...
        for(distance,angle1,center,id) in zip(distance_list,angle_list,center_list,id_list):
            ...
            '''
            x = (distance[0]* (sizeCamX - cX - centerCamX) )/ focalX
            y = (distance[1] * (sizeCamY - cY - centerCamY) )/ focalY
            '''
...
# Define the rotation matrix for a rotation about the X-axis
            ...
            r_angle = round(r_angle, 3)
            rectified_angles.append(r_angle)
            r = R.from_euler('xyz', [0, 0,r_angle], degrees=False)
            ...
            self.br.sendTransform(t)
            temp[0].append('box'+idstr)
            #temp[1].append([list(point_0_1),list(point_0_2)])
            temp[2].append([list(q),list(q)])
        temp[3].append(list(rectified_angles))
        

        
        while True:
            idstr=str(id_list[0])
            idstr1=str(id_list[1])
            idstr2=str(id_list[2])

            try:
                        t2 = self.tf_buffer.lookup_transform('base_link','cam_'+idstr, rclpy.time.Time() )
                        t3 = self.tf_buffer.lookup_transform('base_link','cam_'+idstr1, rclpy.time.Time() )
                        t4 = self.tf_buffer.lookup_transform('base_link','cam_'+idstr2, rclpy.time.Time() )
                        t5 = self.tf_buffer.lookup_transform('base_link','post_pick1_'+idstr, rclpy.time.Time() )
                        t6 = self.tf_buffer.lookup_transform('base_link','post_pick1_'+idstr1, rclpy.time.Time() )
                        t7 = self.tf_buffer.lookup_transform('base_link','post_pick1_'+idstr2, rclpy.time.Time() )
                        t8 = self.tf_buffer.lookup_transform('base_link','post_pick2_'+idstr, rclpy.time.Time() )
                        t9 = self.tf_buffer.lookup_transform('base_link','post_pick2_'+idstr1, rclpy.time.Time() )
                        t10 = self.tf_buffer.lookup_transform('base_link','post_pick2_'+idstr2, rclpy.time.Time() )
                        ...
                        self.br.sendTransform(t)
                        t.child_frame_id = 'obj_'+idstr2
                        t.transform.rotation.x = t4.transform.rotation.x
                        t.transform.rotation.y = t4.transform.rotation.y
                        t.transform.rotation.z = t4.transform.rotation.z
                        t.transform.rotation.w = t4.transform.rotation.w
                        t.transform.translation.x =t4.transform.translation.x 
                        t.transform.translation.y =t4.transform.translation.y
                        t.transform.translation.z =t4.transform.translation.z
                        self.br.sendTransform(t)
                        s=['box'+idstr,'box'+idstr1,'box'+idstr2]
                        msg.data=f'[[[{t2.transform.rotation.x},{t2.transform.rotation.y},{t2.transform.rotation.z},{t2.transform.rotation.w}],[{t3.transform.rotation.x},{t3.transform.rotation.y},{t3.transform.rotation.z},{t3.transform.rotation.w}],[{t4.transform.rotation.x},{t4.transform.rotation.y},{t4.transform.rotation.z},{t4.transform.rotation.w}]],[[{t2.transform.translation.x},{t2.transform.translation.y},{t2.transform.translation.z}],[{t3.transform.translation.x},{t3.transform.translation.y},{t3.transform.translation.z}],[{t4.transform.translation.x},{t4.transform.translation.y},{t4.transform.translation.z}]],{s}]'
                        self.publisher_.publish(msg)
                        self.get_logger().info('Publishing: "%s"' % msg.data)
                        temp[1]=[[list((t5.transform.translation.x,t5.transform.translation.y,t5.transform.translation.z)),list((t8.transform.translation.x,t8.transform.translation.y,t8.transform.translation.z))],[list((t6.transform.translation.x,t6.transform.translation.y,t6.transform.translation.z)),list((t9.transform.translation.x,t9.transform.translation.y,t9.transform.translation.z))],[list((t7.transform.translation.x,t7.transform.translation.y,t7.transform.translation.z)),list((t10.transform.translation.x,t10.transform.translation.y,t10.transform.translation.z))]]
                        break
