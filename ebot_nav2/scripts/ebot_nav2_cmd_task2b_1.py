...
class Docking(Node):
    ...
    def run_docker(self, distance,orientaion,rack_no):
        self.call_ebot_Docker(distance, orientaion,rack_no)

    def call_ebot_Docker(self, distance,orientation, rack_no):
...
class MyBotNavigator(Node):
    def __init__(self):
        ...
        goal_pose.pose.position.x = 0.000000  #-0.029527
        goal_pose.pose.position.y = 4.500000+0.25
        goal_pose.pose.orientation.z =  0.9999743
        goal_pose.pose.orientation.w = 0.0071628
        ...
        docking=Docking()
        docking.run_docker(float(4.349994),float(3.14),'rack_1')
        attach=MagnetON()
        attach.runON('rack1')
        ...
        goal_pose.header.frame_id = 'map'
        goal_pose.header.stamp = self.navigator.get_clock().now().to_msg()
        goal_pose.pose.position.x = 1.05-0.5     #AP1: 0.5, -2.455, 3.14
        goal_pose.pose.position.y = -2.455
        goal_pose.pose.orientation.z = 0.9999997
        goal_pose.pose.orientation.w = 0.0007908
        self.navigator.goToPose(goal_pose)
        i = 0
        while not self.navigator.isTaskComplete():
            ################################################
